...
/*
    ...
    It allows
        Adding new page JavaScript Details
        Viewing page JavaScript Details
        Updating an existing page JavaScript Details
        Deleting an existing page JavaScript Details
*/
...
<!-- ------------------HTML code block rendering the actual page begins. -------------------------------------- -->
        <HTML>
            <HEAD>
                <TITLE>Form Java Script Detail</TITLE>
...
<!-- ------------------SCRIPT tag holding JavaScript under the BODY section. ---------------------------------- -->
    ...
    function setEditMode(id, PageID, ScriptName)
    {       
        ...
        // Assigning The Form Object txtScriptName The Value Of The passed ScriptName parameter
        document.forms[0].txtScriptName.value = ScriptName;
...
<!-- Initialising a form object, which will submit data captured on the form to the PHP processing file. -->
    <FORM ACTION="doSqlMaster.php" METHOD="post" NAME="FrmFormJScriptDetail"  ENCTYPE="MULTIPART/FORM-DATA" >
...
<!-- ------------------Form Hidden Variable to hold the Form IDentity. ------------------------------------- -->
        ...
        <!-- A Hidden Form Element to hold Page ID (Usually PRIMARY KEY COLUMN). This is required in dosql.php (The Processing File) file to identify which record was selected for update (In the WHERE CLAUSE of the UDPATE statement). -->
            <INPUT NAME="hidFormName" SIZE="2" TYPE="hidden" VALUE="FormJSDetail">
...
<!-- ------------------First Child Table to hold the d/e Form objects, to Capture / Control data begins. ----- -->
            ...
            <FIELDSET CLASS="textfield">
                <LEGEND>Form Java Script Detail</LEGEND>

        <!-- HTML Code-spec follows for designing the form fields Script Name, buttons such as Save and Cancel. -->
            <TABLE ALIGN="center"  BORDERCOLOR="" CELLPADDING="2" CELLSPACING="0"    NAME="tblFirstChild">

<!-- ------------------First Child Table: Second Row: Two columns, first column to hold the label and the second to hold the data capture object that captures data. ------------------------------------- -->
            ...
            <!-- Inserting table row that holds table data lable and text box of Primary Key Value -->
                <TR>
                    ...
                    <TD WIDTH="69%" ALIGN="left">
                        <INPUT MAXLENGTH="30" NAME="txtScriptName"  TYPE="file" SIZE="50" CLASS="textfield">
                    </TD>
                </TR>
...
<!-- ------------------PHP code block to retrieve records form the AuthorMaster table. ----------------------- -->
        ...
        <?php
            ...
            /* Storing the query in a variable */
                $query = "SELECT j.*,f.PageTitle FROM FormJavaScriptsDetail j,FormsMaster f where j.PageID=f.PageID ORDER BY SrNo";
...
<!-- ------------------Second Child Table: First Row: Columns to hold the header row along with the Delete button. -------------------------------------------------------------------------------------------------------------- -->
                    ...
                    <!-- Inserting a table row that holds table data of Page Name and Java Script Name fields -->
                        <TR BGCOLOR="#999999" BORDERCOLOR="#333333">
                            <TD WIDTH="52" ALIGN="CENTER"><INPUT NAME="cmdDelete" onClick="setDelMode();" TYPE="button" VALUE="Delete" CLASS="btnStyle"></TD> 
                            <TD  CLASS="BOLDTEXT">Page Name</TD>
                            <TD  CLASS="BOLDTEXT">Java Script</TD>
                        </TR>
...
<!-- ------------------Second Child Table: Second Row: Columns to set data and assign colors to odd and even rows. --------------------------------------------------------------------------------------------------------------- -->
                    ...
                    <TR BORDERCOLOR="#333333" BGCOLOR="#99CCFF" onMouseOver="this.bgColor='#EFEEC9';"  onMouseOut="this.bgColor='#99CCFF';">
                        ...
                        <TD STYLE="cursor:pointer" CLASS="NormalText" onMouseDown="setEditMode(<?php echo $sel_array['SrNo'];?>,<?php echo $sel_array['PageID'];?>,'<?php echo $sel_array['ScriptName'];?>');">
                            <?php echo $sel_array['PageTitle'];?>
                        </TD>
                       
                        <TD STYLE="cursor:pointer" CLASS="NormalText" onMouseDown="setEditMode(<?php echo $sel_array['SrNo'];?>,<?php echo $sel_array['PageID'];?>,'<?php echo $sel_array['ScriptName'];?>');">
                            <?php echo $sel_array['ScriptName'];?>
                        </TD>
