...
//$node_data = mysql_fetch_assoc($result);
	...
	function displayNodeWithLeftID($node_data)
	{
		...
		if (!empty($row)) 
		{
			...
			$result= mysql_query($query) or die("sql query failed");
			$related_with = mysql_fetch_assoc($result);
			echo '<br><strong>'.$related_with['name'].'</strong> is my sibling before me';
		}
		else
		{
			...
			if(!empty($row))
			{
				...
				$result= mysql_query($query) or die("sql query failed");
				$related_with = mysql_fetch_assoc($result);
				echo '<br><strong>'.$related_with['name'].'</strong> is my parent and I am first child of it
				';
			}
			else
			{
				echo 'there is some problem in data';
			}
	...
	function displayNodeWithRightID($node_data)
	{
		...
		if (!empty($row)) 
		{
			...
			$result= mysql_query($query) or die("sql query failed");
			$related_with = mysql_fetch_assoc($result);
			echo '<br><strong>'.$related_with['name'].'</strong> is my sibling after me';
		}
		else
		{
			...
			if(!empty($row))
			{
				...
				$result= mysql_query($query) or die("sql query failed");
				$related_with = mysql_fetch_assoc($result);
				echo '<br><strong>'.$related_with['name'].'</strong> is my parent and I am last child of it';
			}
			else
			{
				echo 'there is some problem in data';
